...
def test_configuration_types(config_files, monkeypatch, config_type, dictionary_item, presence_expected):
    parser = ConfigurationParser(config_files, config_type)
    playbooks = parser.get_ordered_path_items("playbooks")
...
def test_empty_overlay_file(empty_overlay_config_file):
    parser = ConfigurationParser(empty_overlay_config_file)
    assert parser.get_compression() == 'gz'
...
def test_config_nodes_absence(empty_config_file):
    parser = ConfigurationParser(empty_config_file)
...
def test_inexistent_configuration():
    with pytest.raises(FatalError) as error:
        ConfigurationParser("/no/such/file.yml")

    assert "file.yml" in error.value.message
